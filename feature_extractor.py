...
class feature_extractor(nn.Module):
    ...
    def __init__(self):
        ...
        for key, value in state_dict.items():
            if key[:2] != 'fc':
                new_state[key] = value
        self.pretrained_layers.load_state_dict(new_state)
        for p in self.pretrained_layers.parameters():
            p.requires_grad = False'''
 
    def forward(self, eyesLeft, eyesRight, faces, rects):
        x = self.pretrained_layers(eyesLeft, eyesRight, faces, rects) 
